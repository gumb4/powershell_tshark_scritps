...
Measure-Command {
	...
	$head = '"eth.src","ip.src","eth.dst","ip.dst","ntp.reftime","ntp.org","ntp.rec","ntp.xmt"'; #Update Here! Leave the single quote (') surrounding the string
	...
	foreach($pcap in Get-ChildItem -path . -recurse | where{($_ -like ("*.pcapng") -or ($_ -like ("*.pcap")) -or ($_ -like ("*.cap")))}){
		...
		Start-Process -FilePath 'C:\Program Files\Wireshark\tshark.exe' -ArgumentList "-r `"$pcap2open`"","-T fields","-Y ntp","-e eth.src","-e ip.src","-e eth.dst","-e ip.dst","-e ntp.reftime","-e ntp.org","-e ntp.rec","-e ntp.xmt","-E separator=,","-E quote=d" -RedirectStandardOutput $body -NoNewWindow -wait; #update this
