...
Measure-Command {
	...
	$head = '"eth.src","eth.dst","cdp.platform","cdp.deviceid","cdp.number_of_addresses","cdp.portid","cdp.software_version"'; #Update Here! Leave the single quote (') surrounding the string
	...
	foreach($pcap in Get-ChildItem -path . -recurse | where{($_ -like ("*.pcapng") -or ($_ -like ("*.pcap")) -or ($_ -like ("*.cap")))}){
		...
		Start-Process -FilePath 'C:\Program Files\Wireshark\tshark.exe' -ArgumentList "-r `"$pcap2open`"","-T fields","-Y cdp","-e eth.src","-e eth.dst","-e cdp.platform","-e cdp.deviceid","-e cdp.number_of_addresses","-e cdp.portid","-e cdp.software_version","-E separator=,","-E quote=d" -RedirectStandardOutput $body -NoNewWindow -wait; #update this
