...
Measure-Command {
	...
	$head = '"eth.src","ip.src","eth.dst","ip.dst","nbns.flags","nbns.name","nbns.type","nbns.class","nbns.addr"'; #Update Here! Leave the single quote (') surrounding the string
	...
	foreach($pcap in Get-ChildItem -path . -recurse | where{($_ -like ("*.pcapng") -or ($_ -like ("*.pcap")) -or ($_ -like ("*.cap")))}){
		...
		Start-Process -FilePath 'C:\Program Files\Wireshark\tshark.exe' -ArgumentList "-r `"$pcap2open`"","-T fields","-Y nbns","-e eth.src ","-e ip.src ","-e eth.dst ","-e ip.dst ","-e nbns.flags ","-e nbns.name ","-e nbns.type ","-e nbns.class ","-e nbns.addr","-E separator=,","-E quote=d" -RedirectStandardOutput $body -NoNewWindow -wait; #update this
