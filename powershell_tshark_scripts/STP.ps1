...
Measure-Command {
	...
	$head = '"eth.src","eth.dst","stp.version","stp.bridge.hw","stp.port","stp.root.hw","stp.root.cost"'; #Update Here! Leave the single quote (') surrounding the string
	...
	foreach($pcap in Get-ChildItem -path . -recurse | where{($_ -like ("*.pcapng") -or ($_ -like ("*.pcap")) -or ($_ -like ("*.cap")))}){
		...
		Start-Process -FilePath 'C:\Program Files\Wireshark\tshark.exe' -ArgumentList "-r `"$pcap2open`"","-T fields","-Y stp","-e eth.src","-e eth.dst","-e stp.version","-e stp.bridge.hw","-e stp.port","-e stp.root.hw","-e stp.root.cost","-E separator=,","-E quote=d" -RedirectStandardOutput $body -NoNewWindow -wait; #update this
