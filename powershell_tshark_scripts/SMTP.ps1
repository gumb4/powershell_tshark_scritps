...
Measure-Command {
	...
	$head = '"eth.src","ip.src","eth.dst","ip.dst","smtp.req.command","smtp.req.parameter","smtp.response.code","smtp.rsp.parameter","smtp.auth.username","smtp.auth.password","imf.from","imf.to","imf.subject"'; #Update Here! Leave the single quote (') surrounding the string
	...
	foreach($pcap in Get-ChildItem -path . -recurse | where{($_ -like ("*.pcapng") -or ($_ -like ("*.pcap")) -or ($_ -like ("*.cap")))}){
		...
		Start-Process -FilePath 'C:\Program Files\Wireshark\tshark.exe' -ArgumentList "-r `"$pcap2open`"","-T fields","-Y smtp","-e eth.src","-e ip.src","-e eth.dst","-e ip.dst","-e smtp.req.command","-e smtp.req.parameter","-e smtp.response.code","-e smtp.rsp.parameter","-e smtp.auth.username","-e smtp.auth.password","-e imf.from","-e imf.to","-e imf.subject","-E separator=,","-E quote=d" -RedirectStandardOutput $body -NoNewWindow -wait; #update this
