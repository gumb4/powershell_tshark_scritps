...
Measure-Command {
	...
	$head = '"eth.src","ipv6.src","eth.dst","ipv6.dst","dhcpv6.msgtype","dhcpv6.dns_server","dhcpv6.client_fqdn"'; #Update Here! Leave the single quote (') surrounding the string
	...
	foreach($pcap in Get-ChildItem -path . -recurse | where{($_ -like ("*.pcapng") -or ($_ -like ("*.pcap")) -or ($_ -like ("*.cap")))}){
		...
		Start-Process -FilePath 'C:\Program Files\Wireshark\tshark.exe' -ArgumentList "-r `"$pcap2open`"","-T fields","-Y dhcpv6","-e eth.src ","-e ipv6.src ","-e eth.dst ","-e ipv6.dst ","-e dhcpv6.msgtype ","-e dhcpv6.dns_server ","-e dhcpv6.client_fqdn","-E separator=,","-E quote=d" -RedirectStandardOutput $body -NoNewWindow -wait; #update this
