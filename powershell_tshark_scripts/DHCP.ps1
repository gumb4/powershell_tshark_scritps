...
Measure-Command {
	...
	$head = '"eth.src","ip.src","eth.dst","ip.dst","dhcp.option.dhcp","dhcp.option.dhcp_server_id","dhcp.option.subnet_mask"'; #Update Here! Leave the single quote (') surrounding the string
	...
	foreach($pcap in Get-ChildItem -path . -recurse | where{($_ -like ("*.pcapng") -or ($_ -like ("*.pcap")) -or ($_ -like ("*.cap")))}){
		...
		Start-Process -FilePath 'C:\Program Files\Wireshark\tshark.exe' -ArgumentList "-r `"$pcap2open`"","-T fields","-Y dhcp","-e eth.src ","-e ip.src ","-e eth.dst ","-e ip.dst ","-e dhcp.option.dhcp ","-e dhcp.option.dhcp_server_id ","-e dhcp.option.subnet_mask","-E separator=,","-E quote=d" -RedirectStandardOutput $body -NoNewWindow -wait; #update this
