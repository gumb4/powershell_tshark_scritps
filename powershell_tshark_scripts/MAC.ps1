...
Measure-Command {
	...
	$head = '"eth.src","eth.dst"'; #Update Here! Leave the single quote (') surrounding the string
	...
	foreach($pcap in Get-ChildItem -path . -recurse | where{($_ -like ("*.pcapng") -or ($_ -like ("*.pcap")) -or ($_ -like ("*.cap")))}){
		...
		Start-Process -FilePath 'C:\Program Files\Wireshark\tshark.exe' -ArgumentList "-r `"$pcap2open`"","-T fields","-e eth.src","-e eth.dst","-E separator=,","-E quote=d","-E occurrence=f" -RedirectStandardOutput $body -NoNewWindow -wait; #update this
