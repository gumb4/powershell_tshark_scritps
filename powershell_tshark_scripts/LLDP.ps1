...
Measure-Command {
	...
	$head = '"eth.src","eth.dst","lldp.chassis.id.mac","lldp.port.id","lldp.port.desc","lldp.tlv.system.name","lldp.tlv.system.desc","lldp.port.desc"'; #Update Here! Leave the single quote (') surrounding the string
	...
	foreach($pcap in Get-ChildItem -path . -recurse | where{($_ -like ("*.pcapng") -or ($_ -like ("*.pcap")) -or ($_ -like ("*.cap")))}){
		...
		Start-Process -FilePath 'C:\Program Files\Wireshark\tshark.exe' -ArgumentList "-r `"$pcap2open`"","-T fields","-Y lldp","-e eth.src","-e eth.dst","-e lldp.chassis.id.mac","-e lldp.port.id","-e lldp.port.desc","-e lldp.tlv.system.name","-e lldp.tlv.system.desc","-e lldp.port.desc","-E separator=,","-E quote=d" -RedirectStandardOutput $body -NoNewWindow -wait; #update this
