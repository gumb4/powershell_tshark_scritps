...
Measure-Command {
	...
	$head = '"eth.src","ip.src","eth.dst","ip.dst","tds.query","tds.colmetadata.colname","tds.rpc.parameter.name","tds.type_varbyte.data.string","tds.type_varbyte.data.int","tds.type_varbyte.data.float"'; #Update Here! Leave the single quote (') surrounding the string
	...
	foreach($pcap in Get-ChildItem -path . -recurse | where{($_ -like ("*.pcapng") -or ($_ -like ("*.pcap")) -or ($_ -like ("*.cap")))}){
		...
		Start-Process -FilePath 'C:\Program Files\Wireshark\tshark.exe' -ArgumentList "-r `"$pcap2open`"","-T fields","-Y tds","-e eth.src","-e ip.src","-e eth.dst","-e ip.dst","-e tds.query","-e tds.colmetadata.colname","-e tds.rpc.parameter.name","-e tds.type_varbyte.data.string","-e tds.type_varbyte.data.int","-e tds.type_varbyte.data.float","-E separator=,","-E quote=d" -RedirectStandardOutput $body -NoNewWindow -wait; #update this
