...
Measure-Command {
	...
	$head = '"eth.src","ip.src","eth.dst","ip.dst","icmp.type","ip.ttl"'; #Update Here! Leave the single quote (') surrounding the string
	...
	foreach($pcap in Get-ChildItem -path . -recurse | where{($_ -like ("*.pcapng") -or ($_ -like ("*.pcap")) -or ($_ -like ("*.cap")))}){
		...
		Start-Process -FilePath 'C:\Program Files\Wireshark\tshark.exe' -ArgumentList "-r `"$pcap2open`"","-T fields","-Y icmp","-e eth.src","-e ip.src","-e eth.dst","-e ip.dst","-e icmp.type","-e ip.ttl","-E separator=,","-E quote=d" -RedirectStandardOutput $body -NoNewWindow -wait; #update this
