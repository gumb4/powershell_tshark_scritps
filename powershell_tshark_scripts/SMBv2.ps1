...
Measure-Command {
	...
	$head = '"eth.src","ip.src","eth.dst","ip.dst","smb2.nt_status","smb2.cmd","smb2.acct","smb2.domain","smb2.host","smb2.tree","smb2.filename"'; #Update Here! Leave the single quote (') surrounding the string
	...
	foreach($pcap in Get-ChildItem -path . -recurse | where{($_ -like ("*.pcapng") -or ($_ -like ("*.pcap")) -or ($_ -like ("*.cap")))}){
		...
		Start-Process -FilePath 'C:\Program Files\Wireshark\tshark.exe' -ArgumentList "-r `"$pcap2open`"","-T fields","-Y smb2","-e eth.src ","-e ip.src ","-e eth.dst ","-e ip.dst ","-e smb2.nt_status ","-e smb2.cmd ","-e smb2.acct ","-e smb2.domain ","-e smb2.host ","-e smb2.tree ","-e smb2.filename","-E separator=,","-E quote=d" -RedirectStandardOutput $body -NoNewWindow -wait; #update this
